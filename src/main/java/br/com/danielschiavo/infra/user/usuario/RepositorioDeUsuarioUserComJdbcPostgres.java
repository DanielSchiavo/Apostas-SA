...
import br.com.danielschiavo.dominio.usuario.NomeRole;
...
import br.com.danielschiavo.dominio.usuario.Usuario.UsuarioBuilder;
	...
	public Usuario autenticarUsuario(Usuario autenticarUsuario) throws AutenticacaoException, ValidacaoException {
		...
		try (PreparedStatement ps = connection.prepareStatement(sql)) {
			...
			try (ResultSet rs = ps.executeQuery()) {
				...
				UsuarioBuilder usuarioBuilder = Usuario.builder().id(id).email(email).senha(senha);
		...
		try(connection) {
			...
			try(PreparedStatement ps = connection.prepareStatement(sql)) {
				...
				try(ResultSet rs = ps.executeQuery()) {
					...
					UsuarioBuilder usuarioBuilder = Usuario.builder()
												.cpf(cpf)
												.nome(nome)
												.sobrenome(sobrenome)
												.email(email)
